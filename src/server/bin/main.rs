...
impl Server {
    async fn run(self) -> Result<(), io::Error> {
        ...
        loop {
            let start = false;
            let (debounce_tx, mut debounce_rx) = mpsc::channel(300);
            let (network_tx, mut network_rx) = mpsc::channel(300);
            ...
            let server = task::spawn({
                ...
                async move {
                    while let Some(packet) = network_rx.recv().await {
                        // Received a packet
                        debounce_tx.send(packet).await.expect("Unable to talk to debounce");
