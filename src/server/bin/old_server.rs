     ...
     /*   if let Err(_) = timeout(Duration::from_millis(10), rx).await {
        ...
        loop {
            ...
            */
            ...
            //   let mut missing_chunks:Vec<u16>;
            //   missing_chunks.clone_from_slice(&missing_indexes);
            //missing_chunks.retain(|b| b == 0);
            // let missing_chunks = missing_indexes.retain(|&x| x == 0);
            println!("{} {}", get_epoch(), timer_delay);
            if get_epoch() - timer_delay > 10 {
           ...
           // futures::join!(f1, to_send = Some(socket.recv_from(&mut buf));
            to_send = Some(socket.recv_from(&mut buf).await?);
        }
        // If we're here then `to_send` is `None`, so we take a look for the
        // next message we're going to echo back.
